...
function spherePoints() {
  ...
  const midpoint = (a, b) => {
    ...
    const point = vec3.scale(vec3.create(), vec3.add(vec3.create(), vertices[a], vertices[b]), 0.5)
    vertices.push(point)
...
export default function InfiniteMenu({ items = [], scale = 1, backgroundColor = '#111c1a', onItemClick }) {
  const canvasRef = useRef(null)
  const stateRef = useRef({ rotation: quat.create(), velocity: [0, 0], pointer: null, rendered: [], zoom: 0.72, targetZoom: 0.72 })
  ...
  useEffect(() => {
    ...
    Promise.all((items.length ? items : [{ image: '' }]).map((item) => loadImage(item.image))).then((loaded) => {
    ...
    const rotationFromDrag = (deltaX, deltaY) => {
      ...
      state.rotation = quat.normalize(quat.create(), quat.multiply(quat.create(), delta, state.rotation))
    }
    ...
    const handlePointerUp = () => { state.pointer = null; state.targetZoom = 0.72; setIsMoving(false) }
    const handleClick = () => {
    ...
    const handleKeyDown = (event) => {
      ...
      if (event.key === 'Enter' || event.key === ' ') onItemClick?.(items.find((item) => item.id === state.activeId))
    }
    ...
    const render = () => {
      ...
      const radius = Math.min(width, height) * 0.48 * scale * state.zoom
      const camera = Math.max(260, Math.min(width, height) * 0.86 * state.zoom)
      ...
      if (!state.pointer) {
        ...
        if (velocityAmount > 0.0001) {
          const velocityAxis = vec3.normalize(vec3.create(), [state.velocity[1], state.velocity[0], 0])
          const velocityRotation = quat.setAxisAngle(quat.create(), velocityAxis, velocityAmount)
          state.rotation = quat.normalize(quat.create(), quat.multiply(quat.create(), velocityRotation, state.rotation))
        }
        state.velocity[0] *= 0.96
        state.velocity[1] *= 0.96
      }

      const visible = points.map((point, index) => {
        const rotated = rotatePoint(point, state.rotation)
        const depth = (rotated[2] + 1) / 2
        return { point, rotated, depth, index }
      }).sort((a, b) => a.depth - b.depth)

      if (!state.pointer && Math.abs(state.velocity[0]) < 0.02 && Math.abs(state.velocity[1]) < 0.02) {
        const front = visible[visible.length - 1].rotated
        const axis = vec3.cross(vec3.create(), front, [0, 0, 1])
        const distance = Math.min(Math.acos(Math.max(-1, Math.min(1, front[2]))), 0.12)
        if (vec3.length(axis) > 0.0001 && distance > 0.001) {
          vec3.normalize(axis, axis)
          const correction = quat.setAxisAngle(quat.create(), axis, distance * 0.08)
          state.rotation = quat.normalize(quat.create(), quat.multiply(quat.create(), correction, state.rotation))
        }
      }
      ...
      visible.forEach(({ rotated, depth, index }) => {
        ...
        const x = centerX + (rotated[0] * radius * camera) / (camera + rotated[2] * radius)
        const y = centerY - (rotated[1] * radius * camera) / (camera + rotated[2] * radius)
        const discRadius = (8 + Math.pow(depth, 5) * 126) * scale
        const alpha = 0.2 + depth * 0.8
        if (rotated[2] > (nearest?.rotated[2] ?? -2)) nearest = { item, rotated, index }
        ...
        if (image) context.drawImage(image, x - discRadius, y - discRadius, discRadius * 2, discRadius * 2)
        else { context.fillStyle = '#20232d'; context.fill() }
        context.filter = 'none'
        if (item?.accent) {
  ...
  return <div className="infinite-menu" style={{ '--infinite-menu-background': backgroundColor }}>
    <canvas ref={canvasRef} className="infinite-menu-canvas" tabIndex="0" role="img" aria-label="Interactive infinite menu. Use arrow keys to rotate." />
    {activeItem && <>
      <div className={`infinite-menu-title ${isMoving ? 'inactive' : ''}`}><span className="infinite-menu-eyebrow">NOW IN VIEW</span><h2>{activeItem.label}</h2><p>{activeItem.description}</p></div>
      <button className={`infinite-menu-action ${isMoving ? 'inactive' : ''}`} onClick={() => onItemClick?.(activeItem)} aria-label={`Open ${activeItem.label}`}><ArrowIcon /></button>
    </>}
  </div>
}
